...
def sortWords(wordList):    #Sorts the words in alphabetic order
    for key in wordList:
        wordList[key].sort()  


def reduce(candidates, num, c1, c2):
    for word in candidates[num]:
        for constraint in c2[num].keys():   #constraint is the other word that intersects
            isValid = False
            for word2 in candidates[constraint]:    #iterates through the possible other word that intersect
                if word[c2[num][constraint][0]] == word2[c2[num][constraint][1]]:
                    isValid = True
                    break
            if isValid == False:
                candidates[num].remove(word)
                break
                



def solve(candidates, c1, c2):
    while True:
    # for i in range(7):
        for clue in candidates.keys():
            reduce(candidates, clue, c1, c2)
        complete = True
        for clue in candidates.keys():
            if len(candidates[clue]) != 1:
                complete = False
                break
        if complete:
            break
...
def main(args):
    ...
    sortWords(wordlist)
    for key in wordlist:    #convert to a set to remove duplicates
        wordlist[key] = set(wordlist[key])
    # wordlist = sort_by_length(words)
    # sortWords(wordlist)
    print_grid(puzzle)
    constraints = get_constraints(puzzle)
    ...
    candidates = {}
    for clueNum in constraints[0].keys():
        candidates[clueNum] = []
        for word in wordlist[constraints[0][clueNum]]:
            candidates[clueNum].append(word)
    solve(candidates, constraints[0], constraints[1])
    print(candidates)
