...
def ac3(constraints, wordlist):
    ...
    def arc_reduce(x, y):
        ...
        for dx in domain[x].copy():
            satisfied = False
            for dy in domain[y]:
                if satisfies(dx, dy, x, y): # check that it still works
                    satisfied = True
            if not satisfied:
                domain[x].remove(dx)
                change = True
    ...
    print(binary)
    domain = {}
    for x in clues: # all id's
        domain[x] = set(wordlist[unary[x]])
    # worklist = []
    worklist = [(x, y) for x in binary for y in binary[x] if y < x] # add unique pairs of id's that intersect
    # for x in binary:
    #     for y in binary[x]:
    #         if y < x:
    #             worklist.append((x, y))
