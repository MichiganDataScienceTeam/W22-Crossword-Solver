...
def read_words(maxlen=5, min_freq=1, skip=1):
    """Reads clue database and returns wordlistself.

    returns a list of words
    """
    words = defaultdict(int)
    with open("./clues.tsv", "r") as f:
        for line in f:
            if skip > 0:
                skip -= 1
                continue
...
def main(args):
    ...
    solutions = ac3(constraints, wordlist)
    for sol in solutions:
        print(constraints[2][sol]["clue"]["num"], "across" if constraints[2][sol]["across"] else "down")
        print(constraints[2][sol]["clue"]["clue"])
        print(solutions[sol])
